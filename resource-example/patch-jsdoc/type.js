...
function parseTypeExpression(tagInfo) {
    ...
    //todo
    if (["[]"].indexOf(tagInfo.typeExpression.trim()) != -1) {
        tagInfo.typeExpression = "Array";
    }  else if (/^\s*[*\{\[\(].+[\]\}\)\w]\s*$/gim.test(tagInfo.typeExpression)) {
        tagInfo.typeExpression = tagInfo.typeExpression.replace(/^\s*[*\{\[\(].+[\]\}\)\w]\s*$/gim, "Object");
        /*    var inside =tagInfo.typeExpression.trim().match(/^\{(.+)\}$/gim) ;
			var inf="";
